...
const iconBtn =
  "p-2 text-[var(--ds-fg-muted)] hover:text-[var(--ds-fg)] transition-colors";
const preClass =
  "text-sm text-[var(--ds-fg)] font-mono bg-[var(--ds-surface-2)] p-4 rounded overflow-x-auto";
...
export function OCRProcessor({ isOpen, onClose, imageUrl, canvasWidth, canvasHeight, onTextExtracted }: OCRProcessorProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative bg-[var(--ds-bg)] border border-[var(--ds-border)] rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] m-4">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[var(--ds-border)]">
          <div className="flex items-center gap-2">
            <Scan className="h-5 w-5 text-[var(--ds-fg-muted)]" />
            ...
            {result && (
              <>
                <select
                  ...
                  className="px-2 py-1 text-sm border border-[var(--ds-border)]
                    rounded bg-[var(--ds-bg)] text-[var(--ds-fg)]
                ...
                <button onClick={handleCopy} className={iconBtn} title={t("copy")}>
                  {copied ? (
                ...
                <button onClick={handleDownload} className={iconBtn} title={t("download")}>
        ...
        <div className="p-4 overflow-auto max-h-[calc(90vh-140px)]">
          {isInitializing && (
            <div className="flex flex-col items-center justify-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--ds-primary)] mb-4"></div>
              <p className="text-[var(--ds-fg-muted)]">{t("initializing")}</p>
            </div>
          )}

          {!isInitializing && !result && !isProcessing && (
            <div className="flex flex-col items-center justify-center py-12">
              <button onClick={processImage} className={buttonClass("primary", "lg", "gap-2")}>
                <Scan className="h-5 w-5" />
                {t("process")}
              </button>
              <p className="text-sm text-[var(--ds-fg-muted)] mt-4">
                {t("clickToProcess")}
              </p>

              {/* Description and Credits */}
              <div className="mt-8 p-4 bg-[var(--ds-surface-2)] rounded-lg max-w-md">
                <p className="text-sm text-[var(--ds-fg)] mb-3">
                  {t("description")}
                </p>
                <p className="text-xs text-[var(--ds-fg-muted)] mb-1">
                  {t("credit")}
                </p>
                <a
          ...
          {isProcessing && (
            <div className="flex flex-col items-center justify-center py-12">
              <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[var(--ds-primary)] mb-4"></div>
              <p className="text-[var(--ds-fg-muted)]">{t("processing")}</p>
            </div>
          )}
          ...
          {result && (
            <div>
              {outputFormat === "text" && (
                <pre className={`${preClass} whitespace-pre-wrap`}>
          ...
          <div className="p-4 border-t border-[var(--ds-border)]">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={processImage} className={buttonClass("secondary", "sm")}>
                  {t("process")}
                </button>
